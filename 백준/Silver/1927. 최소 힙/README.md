...
heapq 모듈을 사용할 줄 아니 매우 쉽게 풀렸다.
모듈 사용을 지양하고 직접 클래스를 생성하는게 옳다고 생각했는데 모듈을 적극 활용하는 것도 시간복잡도 완화에 도움이 되는 것 같다.

hq.heappush(heap, x)을 사용하여 x를 hq에 삽입하였고, 
hq.heappop(heap)으로 최소값을 삭제 및 반환할 수 있었다. 
